...
The parent-ID/name create lock and profile/path metadata lock coordinate only this application process. Google Drive does not enforce globally unique names, so cross-process duplicate names remain possible; a later authoritative lookup reports that state as ambiguity instead of choosing or deleting an object. Recursive backup-file listing, backup upload/download, preview, and execution remain unavailable, and Google Drive remains `IsImplemented = false`.
