...
* **Sync tab** - backup-run sync with a local/mounted folder or an SFTP server:
  ...
  * Provider behavior is described by one capability catalog. Google Drive is implemented for sync as of Milestone V; WebDAV and OneDrive remain declared but unavailable.
...
* Google Drive backup synchronization is activated as of Milestone V, closed on 2026-08-20 after controlled live acceptance. Preview Sync and Sync Now work against a connected account with a ready backup folder. Upload stays create-only and download stays no-overwrite. The [Google Drive developer setup guide](docs/google-drive-developer-setup.md) explains private development configuration and the recorded acceptance procedures; normal users do not create a Cloud project, and personal credentials or tokens must never be committed.
  * Type-safe provider selection exposes `LocalFolder`, `Sftp`, and `GoogleDrive`. All three can preview and execute sync; WebDAV and OneDrive remain unavailable.
...
Named remote configurations are stored in SQLite. Local Folder, SFTP, and Google Drive profiles are usable now; WebDAV and OneDrive profiles remain unavailable roadmap values.
...
Milestone O introduced `GoogleDriveRemoteFileSystem` as a validation-only boundary. Validation restores protected authentication silently, resolves the configured root directly by its authoritative ID, checks My Drive folder and child capabilities, and never creates a probe or mutates Drive. Milestone P subsequently added its narrow listing and text-metadata operations without activating a sync provider.
...
* [x] `RootExistsAsync`
* [x] `ListRunFolderNamesAsync`
* [x] `FolderExistsAsync`
* [x] `ReadTextFileAsync`
* [x] The safe metadata-write operations introduced in milestone M
* [x] Support API pagination and request only required fields
* [x] Use folder/file IDs
* [x] Ignore folders without `manifest.json`
* [x] Handle unreadable manifests as warnings
* [x] Preserve existing `SyncEngine` behavior
* [x] Support `.gamesave-sync/sync-log.json`
